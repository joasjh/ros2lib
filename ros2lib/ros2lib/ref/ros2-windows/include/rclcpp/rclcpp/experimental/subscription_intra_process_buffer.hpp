...
public:
  RCLCPP_SMART_PTR_DEFINITIONS(SubscriptionIntraProcessBuffer)
      ...
      // 추가: 복사 생성자
      SubscriptionIntraProcessBuffer(const SubscriptionIntraProcessBuffer& other)
      : SubscriptionROSMsgIntraProcessBuffer<ROSMessageType, ROSMessageTypeAllocator, ROSMessageTypeDeleter>(other),
      subscribed_type_allocator_(other.subscribed_type_allocator_),
      subscribed_type_deleter_(other.subscribed_type_deleter_),
      buffer_(other.buffer_) // 복사할 수 있는지 확인 필요
  {
  }

  // 추가: 이동 생성자
  SubscriptionIntraProcessBuffer(SubscriptionIntraProcessBuffer&& other) noexcept
      : SubscriptionROSMsgIntraProcessBuffer<ROSMessageType, ROSMessageTypeAllocator, ROSMessageTypeDeleter>(std::move(other)),
      subscribed_type_allocator_(std::move(other.subscribed_type_allocator_)),
      subscribed_type_deleter_(std::move(other.subscribed_type_deleter_)),
      buffer_(std::move(other.buffer_)) // 이동할 수 있는지 확인 필요
  {
  }
