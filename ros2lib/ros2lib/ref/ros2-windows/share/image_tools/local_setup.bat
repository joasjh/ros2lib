...
set "AMENT_ENVIRONMENT_HOOKS[image_tools]="

:: these are the environment hooks of this package
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[image_tools] "%AMENT_CURRENT_PREFIX%\share\image_tools\environment\ament_prefix_path.bat"
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[image_tools] "%AMENT_CURRENT_PREFIX%\share\image_tools\environment\path.bat"
...
if not defined AMENT_RETURN_ENVIRONMENT_HOOKS (
  ...
  for %%f in ("%AMENT_ENVIRONMENT_HOOKS[image_tools]:;=";"%") do (
    call:call_file %%f
  )
