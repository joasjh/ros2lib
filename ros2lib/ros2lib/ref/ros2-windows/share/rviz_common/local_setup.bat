...
set "AMENT_ENVIRONMENT_HOOKS[rviz_common]="

:: these are the environment hooks of this package
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[rviz_common] "%AMENT_CURRENT_PREFIX%\share\rviz_common\environment\ament_prefix_path.bat"
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[rviz_common] "%AMENT_CURRENT_PREFIX%\share\rviz_common\environment\path.bat"
...
if not defined AMENT_RETURN_ENVIRONMENT_HOOKS (
  ...
  for %%f in ("%AMENT_ENVIRONMENT_HOOKS[rviz_common]:;=";"%") do (
    call:call_file %%f
  )
