...
set "AMENT_ENVIRONMENT_HOOKS[ros_testing]="

:: these are the environment hooks of this package
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[ros_testing] "%AMENT_CURRENT_PREFIX%\share\ros_testing\environment\ament_prefix_path.bat"
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[ros_testing] "%AMENT_CURRENT_PREFIX%\share\ros_testing\environment\path.bat"
...
if not defined AMENT_RETURN_ENVIRONMENT_HOOKS (
  ...
  for %%f in ("%AMENT_ENVIRONMENT_HOOKS[ros_testing]:;=";"%") do (
    call:call_file %%f
  )
