...
set "AMENT_ENVIRONMENT_HOOKS[composition]="

:: these are the environment hooks of this package
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[composition] "%AMENT_CURRENT_PREFIX%\share\composition\environment\ament_prefix_path.bat"
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[composition] "%AMENT_CURRENT_PREFIX%\share\composition\environment\path.bat"
...
if not defined AMENT_RETURN_ENVIRONMENT_HOOKS (
  ...
  for %%f in ("%AMENT_ENVIRONMENT_HOOKS[composition]:;=";"%") do (
    call:call_file %%f
  )
