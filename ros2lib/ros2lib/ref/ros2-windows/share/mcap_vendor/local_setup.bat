...
set "AMENT_ENVIRONMENT_HOOKS[mcap_vendor]="

:: these are the environment hooks of this package
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[mcap_vendor] "%AMENT_CURRENT_PREFIX%\share\mcap_vendor\environment\ament_prefix_path.bat"
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[mcap_vendor] "%AMENT_CURRENT_PREFIX%\share\mcap_vendor\environment\path.bat"
...
if not defined AMENT_RETURN_ENVIRONMENT_HOOKS (
  ...
  for %%f in ("%AMENT_ENVIRONMENT_HOOKS[mcap_vendor]:;=";"%") do (
    call:call_file %%f
  )
