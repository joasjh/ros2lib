...
set "AMENT_ENVIRONMENT_HOOKS[rcl_lifecycle]="

:: these are the environment hooks of this package
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[rcl_lifecycle] "%AMENT_CURRENT_PREFIX%\share\rcl_lifecycle\environment\ament_prefix_path.bat"
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[rcl_lifecycle] "%AMENT_CURRENT_PREFIX%\share\rcl_lifecycle\environment\path.bat"
...
if not defined AMENT_RETURN_ENVIRONMENT_HOOKS (
  ...
  for %%f in ("%AMENT_ENVIRONMENT_HOOKS[rcl_lifecycle]:;=";"%") do (
    call:call_file %%f
  )
